    ...
    try {
      ...
      if (!gotConversation) {
        gotConversation = await Conversation.create({
          ...
          message: [] // Ensure the message array exists
        });
      }
      ...
      if (newMessage) {
        ...
        await gotConversation.save(); // ✅ Save the conversation after adding the message
      }
...
export const getOrCreateConversation = async (req, res) => {
  try {
    ...
    res.status(200).json({ conversationId: conversation._id  , message:"Conversation Created"});
  } catch (error) {
