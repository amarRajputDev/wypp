...
export const sendFriendRequest = async (req, res) => {
  ...
  export const getFriendRequests = async (req, res) => {
    try {
      ...
      const receivedRequests = await FriendRequest.find({ receiver: userId, status: "pending" }).populate("sender" , "username profilePic _id");
      const sentRequests = await FriendRequest.find({ sender: userId, status: "pending" }).populate("receiver" , "username profilePic _id");
  ...
  export const deleteFriendRequest = async (req, res) => {
    try {
      ...
      // Find the user and remove the friend from their outgoing requests list
      const user = await User.findByIdAndUpdate(
        userId,
      ...
      res.status(200).json({ message: "Friend request deleted successfully", outgoingRequests: user.outgoingRequests });
    } catch (error) {
