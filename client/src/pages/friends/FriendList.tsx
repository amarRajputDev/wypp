...
export default function FriendsList({incomingRe , outgoingRe}:{incomingRe:Request[] | [] , outgoingRe:Request[] | []}) {
  ...
  //   { id: 2, name: "Maria Garcia", avatar: "/placeholder.svg?height=40&width=40", status: "offline" },
  //   { id: 3, name: "James Wilson", avatar: "/placeholder.svg?height=40&width=40", status: "online" },
  //   { id: 4, name: "Sarah Brown", avatar: "/placeholder.svg?height=40&width=40", status: "offline" },
  // ])
  ...
  const handleRemoveFriend = async (id:string) => {
    ...
    try {
      ...
      if (res.status === 200) {
        setFriends(friends.filter((friend:any) => friend._id !== id))
          ...
          {incomingReq.length === 0 ? (
            ...
            <div className="divide-y ">
              {incomingReq.map((request) => (
                <FriendListItem key={request._id} handleAcceptRequest={handleAcceptRequest} handleRejectRequest={handleRejectRequest} request={request} type={"incoming"} />
              ))}
...
{outgoingReq.length === 0 ? (
            ...
            <div className="divide-y ">
              {outgoingReq.map((request) => (
                <FriendListItem key={request._id} handleAcceptRequest={handleAcceptRequest} handleRejectRequest={handleRejectRequest} request={request}  />
              ))}
...
function FriendListItem({ request, handleAcceptRequest, handleRejectRequest, type = "outgoing" }) {

  const [erase, seterase] = useState(false)

  return (
    <motion.div animate={{scale:erase? 0 : 1}}>
      <div className="py-3 flex items-center justify-between">
        ...
        {type === "incoming" ? (
          ...
          <button className="btn btn-sm btn-ghost btn-circle" onClick={() => {seterase(true); handleRejectRequest(request._id)}}>
            <X size={16} />
          </button>
        )}
      </div>
